...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		removeAll(first, el, null);
	}

	private void removeAll(Node ref, E el, Node before) {
		if (ref == first && (E) ref.data == el) {
			first = first.next;
			ref = ref.next;
			n--;
			removeAll(ref, el, before);
		} else if (ref != null && (E) ref.data == el) {
			before.next = ref.next;
			Node newNext = ref.next;
			ref.next = null;
			ref = newNext;
	...
	public void duplicateAll(E el) {
		...
		// with first as an argument. It must be recursive, no loop allowed.
		if (first != null) {
			duplicateAll(first, el, first.next);
		}
	}

	private void duplicateAll(Node ref, E el, Node after) {
		if (ref != null && (E) ref.data == el) {
			Node duplicateIt = new Node(el, after);
			ref.next = duplicateIt;
			n++;
			ref = after;
			if (ref != null) {
				after = ref.next;
